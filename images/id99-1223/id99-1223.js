window.addEventListener('load', function() {
 ...
 //     console.log(dc)
      ...
      let tick = 0;
      var madeby = [
        new MadeBy("December 23rd 2023",canvas.width + 600, 140),
      ];
      /*
      ...
      function animate() {
        ...
        switch(state){
          case 0:
              ...
              pos.x += 100;
              if (pos.x > 5*canvas.width/6){
                pos.x = 50;
                pos.y += 100;
              }
              if (pos.y > 5*canvas.height/6) {
                state = 1;
              }
            break;
          case 1:
          ...
          case 2:
            ...
            if (xmax > canvas.height-80){
              state = 5;
                ...
                var color = 'rgb(' + r + ',' + g + ',' + b + ')';
                for(j=0;j<400;j++){
                  explosion2.push(new ParticleEx(canvas.width/2 - 30 + 60*Math.random(),canvas.height - 60 + 60*Math.random(), color));
                }
            }
            if (alien.length == 0){
              state = 6;
          ...
          case 6:
            ...
            break;
          case 7:
            var love = {x: canvas.width/2, y:canvas.height/2, size:5, angle:3.14, mangle:3.14};
            var angle = tick / 2;
            dc.save();
            ...
            dc.save();
            dc.translate(love.x, love.y+12); // center of window
            var scale = love.size * (1 + Math.sin(love.angle + angle/100));
            dc.scale(scale, scale);
            dc.fillStyle = 'green';
            dc.font = "8px Russo One";
            var text = "Coding";
            dc.fillText(text, -dc.measureText(text).width/2, 0);
            dc.restore();
        ...
        if (explosion2.length == 0){
          ...
          dc.closePath();
            ...
            dc.fillStyle = '#a0a0a0'; 
            dc.fillRect(canvas.width/2 - 50, canvas.height-50, 100, 40);
            dc.fillRect(canvas.width/2 - 3, canvas.height-60, 6, 10);

            if (mouse.down){
              var angle = -Math.atan2(canvas.height-60 - mouse.y, mouse.x - canvas.width/2);
              bullit.push(new ParticleRay(canvas.width/2, canvas.height-60, angle));
            }
          } 
          tick++;
